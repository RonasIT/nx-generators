...
import { askQuestion, getImportPathPrefix, getLibraryDetailsByName, normalizeLibPath } from '../../shared/utils';
...
export async function libRenameGenerator(tree: Tree, options: LibRenameGeneratorSchema): Promise<void> {
  ...
  const newLibImportPath = normalizeLibPath(libPathSegments.join('/'));
  const fullLibraryPath = `${getImportPathPrefix(tree)}/${newLibImportPath}`;
  const fullLibraryName = newLibImportPath.split('/').join('-');

  execSync(
    `npx nx g mv --projectName=${currentLibraryName} --newProjectName=${fullLibraryName} --destination=${destLibraryPath} --importPath=${fullLibraryPath}`,
    { stdio: 'inherit' },
  );
}
